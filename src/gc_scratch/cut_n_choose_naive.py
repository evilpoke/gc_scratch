...
from gates import AccessRejectedGate, InterWire, XORGate, getallinputwires
...
def openv_nonrec(circ_to_be_opened, Deltakey):
    ...
    while True:
        for w in wires:
            ...
            for t in targetgates:
                ...
                if isinstance(t, XORGate):
                    supposed_true_label_of_first = t.input_gates[0].possiblelables[1]
                    supposed_false_label_of_first = t.input_gates[0].possiblelables[0]

                    supposed_true_label_of_second = t.input_gates[1].possiblelables[1]
                    supposed_false_label_of_second = t.input_gates[1].possiblelables[0]
                    
                    supposed_label_result = {1: None, 0: None}
                    
                    supposed_label_result[0] = xoring_bytearray( supposed_false_label_of_first , supposed_false_label_of_second)
                    supposed_label_result[1] = xoring_bytearray(Deltakey, supposed_label_result[0] )
...
def verify_functional_equality(circ_to_be_opened, Deltakey):
    ...
    openv_nonrec(circ_to_be_opened, Deltakey)
